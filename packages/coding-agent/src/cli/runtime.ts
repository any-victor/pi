...
import { hasProjectTrustInputs, type ProjectTrustStore } from "../core/trust-manager.ts";
...
import { type Args, type Mode, parseArgs, printHelp } from "./args.ts";
...
import { type CliServiceProvider, DefaultCliServiceProvider } from "./service-provider.ts";
...
export interface MainOptions {
	...
	services?: Partial<CliServiceProvider>;
}
...
export interface CliRuntime {
	...
	provider: CliServiceProvider;
}

export async function createCliRuntime(args: string[], options?: MainOptions): Promise<CliRuntime | undefined> {
	const provider = Object.assign(new DefaultCliServiceProvider(), options?.services);
	resetTimings();
	...
	const { migratedAuthProviders: migratedProviders, deprecationWarnings } = provider.runMigrations(process.cwd());
	time("runMigrations");

	const cwd = process.cwd();
	const agentDir = getAgentDir();
	const startupSettingsManager = provider.createSettingsManager(cwd, agentDir);
	...
	const trustStore = provider.createTrustStore(agentDir);
	...
	const authStorage = provider.createAuthStorage();
	const createRuntime: CreateAgentSessionRuntimeFactory = async ({
		...
		const runtimeSettingsManager = provider.createSettingsManager(cwd, agentDir, { projectTrusted });
		const services = await provider.createServices({
			cwd,
			agentDir,
			authStorage,
		...
		const created = await provider.createSessionFromServices({
			services,
			sessionManager,
			sessionStartEvent,
	...
	time("createRuntime");
	const runtime = await provider.createRuntime(createRuntime, {
		cwd: sessionManager.getCwd(),
		agentDir,
		sessionManager,
	});
	...
	const { settingsManager, modelRegistry, resourceLoader } = services;
	provider.configureHttpDispatcher(settingsManager.getHttpIdleTimeoutMs());
	...
	provider.initTheme(settingsManager.getTheme(), appMode === "interactive");
	time("initTheme");
...
export async function runCliMode(state: CliRuntime): Promise<void> {
	const {
		appMode,
		runtime,
		migratedProviders,
	...
	if (appMode === "rpc") {
		printTimings();
		await provider.runRpcMode(runtime);
	} else if (appMode === "interactive") {
		const interactiveMode = provider.createInteractiveMode(runtime, {
			migratedProviders,
			modelFallbackMessage,
			initialMessage,
			initialImages,
		...
		printTimings();
		const exitCode = await provider.runPrintMode(runtime, {
...
export async function runCli(args: string[], options?: MainOptions): Promise<void> {
	const cliRuntime = await createCliRuntime(args, options);
	if (!cliRuntime) return;
	await runCliMode(cliRuntime);
}
